...
import nl.grons.metrics5.scala.ActorInstrumentedLifeCycleSpec._
...
object ActorInstrumentedLifeCycleSpec {
  ...
  private val NonLoggingActorSystem = ActorSystem("lifecycle-spec", ConfigFactory.parseMap(Map("akka.loglevel" -> "OFF").asJava))

  private val TestMetricRegistry = new io.dropwizard.metrics5.MetricRegistry()
...
class ActorInstrumentedLifeCycleSpec extends TestKit(NonLoggingActorSystem) with FunSpecLike with ImplicitSender with Matchers with ScalaFutures with BeforeAndAfterAll {
  ...
  def report = {
    TestMetricRegistry
      .getGauges(MetricFilter.startsWith("nl.grons.metrics5.scala")).asScala
      .headOption
