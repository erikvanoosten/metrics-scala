...
import io.dropwizard.metrics5.{Metric, MetricFilter, MetricName, MetricRegistry}
...
class ActorMetricsSpec extends FunSpec with OneInstancePerTest {
  ...
  describe("A counter actor") {
    it("invokes original receive and increments counter on new messages") {
      ...
      val counterName = "nl.grons.metrics5.scala.ActorMetricsSpec.CounterTestActor.receiveCounter"
    ...
    it("can override metric name") {
      ...
      val counterName = "nl.grons.metrics5.scala.ActorMetricsSpec.CounterTestActorNameOverride.overrideReceiveCounter"
  ...
  describe("A timer actor") {
    it("invokes original receive and times message processing") {
      ...
      val timerName = "nl.grons.metrics5.scala.ActorMetricsSpec.TimerTestActor.receiveTimer"
    ...
    it("can override metric name") {
      ...
      val timerName = "nl.grons.metrics5.scala.ActorMetricsSpec.anon.something-else"
  ...
  describe("An exception meter actor") {
    it("invokes original receive and meters thrown exceptions") {
      val ref = TestActorRef(new ExceptionMeterTestActor)
      val meterName = "nl.grons.metrics5.scala.ActorMetricsSpec.ExceptionMeterTestActor.receiveExceptionMeter"
  ...
  describe("A composed actor") {
    it("counts and times processing of messages") {
      ...
      val counterName = "nl.grons.metrics5.scala.ActorMetricsSpec.ComposedActor.receiveCounter"
      val timerName = "nl.grons.metrics5.scala.ActorMetricsSpec.ComposedActor.receiveTimer"
      val meterName = "nl.grons.metrics5.scala.ActorMetricsSpec.ComposedActor.receiveExceptionMeter"
  ...
  private def nameFilter(filterName: String) = new MetricFilter {
    override def matches(name: MetricName, metric: Metric): Boolean = name.getKey == filterName
  }
}
