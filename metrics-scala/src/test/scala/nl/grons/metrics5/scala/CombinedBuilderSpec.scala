...
class CombinedBuilderSpec extends FunSpec with OneInstancePerTest {
  ...
  describe("InstrumentedBuilder combined with CheckedBuilder") {
    it("uses owner class as metric base name") {
      ...
      verify(combinedBuilder.metricRegistry).counter(MetricName.build("nl.grons.metrics5.scala.CombinedBuilderSpec.CombinedBuilder.cnt"))

      val check = combinedBuilder.createBooleanHealthCheck { true }
      verify(combinedBuilder.registry).register("nl.grons.metrics5.scala.CombinedBuilderSpec.CombinedBuilder.test", check)
    }

    it("supports overriding the metric base name") {
      val combinedBuilder = new CombinedBuilder {
        override lazy val metricBaseName: MetricName = MetricName.build("OverriddenBaseName")
      }

      combinedBuilder.createCounter()
      verify(combinedBuilder.metricRegistry).counter(MetricName.build("OverriddenBaseName.cnt"))
