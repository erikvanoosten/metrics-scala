...
import io.dropwizard.metrics5.health.{HealthCheck, HealthCheckRegistry}
...
trait CheckedBuilder extends BaseBuilder {
  /**
   * The [[io.dropwizard.metrics5.health.HealthCheckRegistry]] where created metrics are registered.
   */
  def registry: HealthCheckRegistry

  /**
    * Converts a code block to a [[io.dropwizard.metrics5.health.HealthCheck]] and registers it.
    *
    * Use it as follows:
    * {{{
    ...
    * [[io.dropwizard.metrics5.health.HealthCheck.Result]], or simply `Unit`.
    *
    ...
    *    unhealthy. The embedded value (after applying `.toString`) or throwable is used as (un)healthy message.
    *  - If the check result is of type [[io.dropwizard.metrics5.health.HealthCheck.Result]], the result is passed
    *    unchanged.
    ...
    *    unhealthy message.
    *
    * By default health check (and metric) names are prefixed with the name of the current class. The prefix is called
    * the metric base name. It is possible to override the metric base name. See [[BaseBuilder]] for instructions.
    *
  ...
  def healthCheck[T](name: String, unhealthyMessage: String = "Health check failed")(checker: => T)(implicit toMagnet: ByName[T] => HealthCheckMagnet): HealthCheck = {
    ...
    registry.register(metricBaseName.resolve(name).getKey, check)
    check
  }
}
...
object HealthCheckMagnet {
  ...
  implicit def fromBooleanCheck[T](checker: ByName[T])(implicit convert: T => Boolean): HealthCheckMagnet = {
    ...
    new HealthCheckMagnet {
      def apply(unhealthyMessage: String): HealthCheck = new HealthCheck() {
  ...
  /**
   * Magnet for checkers returning a [[io.dropwizard.metrics5.health.HealthCheck.Result]].
   */
...
object ByName {
  ...
  /**
   * Implicitly converts a by-name `expression` of type `T` into an instance of [[nl.grons.metrics5.scala.ByName]].
   */
