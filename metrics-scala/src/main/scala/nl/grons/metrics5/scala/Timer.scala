...
import io.dropwizard.metrics5.{Snapshot, Timer => DropwizardTimer}
...
/**
  ...
  * Example usage:
  * {{{
  *   class Example(val db: Db) extends DefaultInstrumented {
...
class Timer(private[scala] val metric: DropwizardTimer) {
  ...
  /**
   * A timing [[io.dropwizard.metrics5.Timer.Context]],
